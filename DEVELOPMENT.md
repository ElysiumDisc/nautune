...
```bash
# Example: bump from 8.5.1 to 8.5.2
sed -i 's/version: 8.5.1+1/version: 8.5.2+1/' pubspec.yaml
sed -i "s/8.5.1+1/8.5.2+1/" lib/app_version.dart
```
...
mkdir -p dist && \
ARCH=x86_64 ./appimagetool AppDir dist/Nautune-x86_64-8.5.1.AppImage
```
